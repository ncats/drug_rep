...
This file **identify the compound-gene relationships** for the 7170 compounds (8971 compounds --> data cleaning & keeping only compounds with complete bioassay data --> 7,170 compounds) using data from Pharos and Drug Repurposing Hub. **7,030** compounds with smiles (6503 from parent smiles list and 527 from the aggregated tox21 data). 
...
# SMILES for original tox21 data; [use Ruili's parent smiles list]
```{r}
...
# 7170 compounds = 6503 not missing smiles + 667 missing smiles
...
## use the Tox21 aggregated data to see if we can find smiles for the "missing" data above
```{r}
...
#find the smiles for the "missing" data (667 - 140 missing = 527 not missing)
data_wo_smiles = 
...
#check the characteristics of the 140 cmps with missing smiles
```{r, eval = FALSE}
data_no_smiles = 
  left_join(missing, aggregated_tox21_unique, by = "cas") %>% 
  distinct() %>% 
...
#check how many of these compounds are drugs/EPA/NTP
```{r}
