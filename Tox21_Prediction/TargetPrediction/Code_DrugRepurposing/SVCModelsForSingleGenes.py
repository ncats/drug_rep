...
for k in range(N):
    ...
    # Organize results for all models and genes and create a dataframe to save the results
means_array = []
stds_array = []
# bestscores_array = []
# bestparams_array = []
for gene in range(N):
    means_array.append(Singlegene_svc[gene].cv_results_['mean_test_score'])
    stds_array.append(Singlegene_svc[gene].cv_results_['std_test_score'])
#     bestscores_array.append(Singlegene_svc[gene].best_score_)
#     bestparams_array.append(Singlegene_svc[gene].best_params_)
my_columns = [str(element) for element in Singlegene_svc[gene].cv_results_['params']]
my_rows = list(df_gene[sum_col.sort_values(ascending=False).index[:N]].columns.values)
df_SVCModelsForSingleGenes = pd.DataFrame(means_array,columns=my_columns,index = my_rows)

# save results
df_SVCModelsForSingleGenes.to_csv('SVCModelsForSingleGenes.csv')