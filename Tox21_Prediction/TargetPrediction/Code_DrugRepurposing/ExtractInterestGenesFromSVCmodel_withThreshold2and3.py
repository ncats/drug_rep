...
for k in range(N):
    ...
    print('start svc prediction for target gene', df_gene_single.name)
    clf = SVC(kernel='rbf', gamma=0.5, C=15, probability=True).fit(x_train, y_train)
...
df_interest_gene_svc.to_csv('InterestGeneForCompoundsSVC_withThreshold2.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureSVC2.csv')
df_threshold.to_csv('ThresholdSVC2.csv')
...
for k in range(N):
    ...
    print('start svc prediction for target gene', df_gene_single.name)
    clf = SVC(kernel='rbf', gamma=0.5, C=10, probability=True).fit(x_train, y_train)
    y_pro = np.array(clf.predict_proba(norm_train_features))[:,1]
    y_pro_array.append(y_pro)
...
df_interest_gene_svc.to_csv('InterestGeneForCompoundsSVC_withThreshold3.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureSVC3.csv')
df_threshold.to_csv('ThresholdSVC3.csv')