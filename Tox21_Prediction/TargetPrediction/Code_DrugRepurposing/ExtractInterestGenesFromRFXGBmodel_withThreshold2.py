...
for k in range(N):
    ...
    clf = RandomForestClassifier(bootstrap = False, max_depth = 6, max_features = 'auto', min_samples_leaf = 2, min_samples_split = 3, 
    ...
    clf = XGBClassifier(colsample_bytree=0.6, max_depth=3, min_child_weight=3, reg_alpha=1, subsample= 0.8).fit(x_train, y_train)
    y_pro = np.array(clf.predict_proba(norm_train_features))[:,1]
...
df_interest_gene_rf.to_csv('InterestGeneForCompoundsRF2.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureRF2.csv')
df_threshold.to_csv('ThresholdRF2.csv')
...
df_interest_gene_xgb.to_csv('InterestGeneForCompoundsXGB2.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureXGB2.csv')
df_threshold.to_csv('ThresholdXGB2.csv')