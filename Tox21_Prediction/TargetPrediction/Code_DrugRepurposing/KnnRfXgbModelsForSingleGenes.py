...
knn_parameters = {
        ...
        'n_neighbors': [3, 5, 7, 9, 11, 13, 17, 25, 31, 33, 35],
        'p': [1,2,3]
    }

xgb_parameters = {
        'max_depth':[3,5,7,9,13,15,17,19],
        'min_child_weight':[1,3,5,7],
        #'classifier__gamma':[i/10.0 for i in range(0,2)],
...
rf_parameters = {
        'n_estimators': [2, 5, 8, 11, 14, 17, 20, 23, 26],
        'max_depth': [3, 6, 9, 12],
...
for k in range(N):
    ...
    print('start knn model selection for target gene', df_gene_single.name)
    Singlegene_knn.append(model_selection(knn_clf, knn_parameters, x_train, y_train, scoring = 'roc_auc', cv=5, GridSearch = True, n_jobs=-1))

    print('start rf model selection for target gene', df_gene_single.name)
    Singlegene_rf.append(model_selection(rf_clf, rf_parameters, x_train, y_train, scoring = 'roc_auc', cv=5, GridSearch = True, n_jobs=-1))

    print('start xgb model selection for target gene', df_gene_single.name)
    Singlegene_xgb.append(model_selection(xgb_clf, xgb_parameters, x_train, y_train, scoring = 'roc_auc', cv=5, GridSearch = True, n_jobs=-1))




# Organize results for all knn models and genes and create a dataframe to save the results
knnmeans_array = []
knnstds_array = []
# bestscores_array = []
# bestparams_array = []
for gene in range(N):
    knnmeans_array.append(Singlegene_knn[gene].cv_results_['mean_test_score'])
    knnstds_array.append(Singlegene_knn[gene].cv_results_['std_test_score'])
#     bestscores_array.append(Singlegene_svc[gene].best_score_)
#     bestparams_array.append(Singlegene_svc[gene].best_params_)
my_columns = [str(element) for element in Singlegene_knn[gene].cv_results_['params']]
my_rows = list(df_gene[sum_col.sort_values(ascending=False).index[:N]].columns.values)
df_knnModelsForSingleGenes = pd.DataFrame(knnmeans_array,columns=my_columns,index = my_rows)

# save knn results
df_knnModelsForSingleGenes.to_csv('knnModelsForSingleGenes.csv')

# Organize results for all rf models and genes and create a dataframe to save the results
rfmeans_array = []
rfstds_array = []
# bestscores_array = []
# bestparams_array = []
for gene in range(N):
    rfmeans_array.append(Singlegene_rf[gene].cv_results_['mean_test_score'])
    rfstds_array.append(Singlegene_rf[gene].cv_results_['std_test_score'])
#     bestscores_array.append(Singlegene_svc[gene].best_score_)
#     bestparams_array.append(Singlegene_svc[gene].best_params_)
my_columns = [str(element) for element in Singlegene_rf[gene].cv_results_['params']]
df_rfModelsForSingleGenes = pd.DataFrame(rfmeans_array,columns=my_columns,index = my_rows)

# save rf results
df_rfModelsForSingleGenes.to_csv('rfModelsForSingleGenes.csv')

# Organize results for all xgb models and genes and create a dataframe to save the results
xgbmeans_array = []
xgbstds_array = []
# bestscores_array = []
# bestparams_array = []
for gene in range(N):
    xgbmeans_array.append(Singlegene_xgb[gene].cv_results_['mean_test_score'])
    xgbstds_array.append(Singlegene_xgb[gene].cv_results_['std_test_score'])
#     bestscores_array.append(Singlegene_svc[gene].best_score_)
#     bestparams_array.append(Singlegene_svc[gene].best_params_)
my_columns = [str(element) for element in Singlegene_xgb[gene].cv_results_['params']]
df_xgbModelsForSingleGenes = pd.DataFrame(xgbmeans_array,columns=my_columns,index = my_rows)

# save xgb results
df_xgbModelsForSingleGenes.to_csv('xgbModelsForSingleGenes.csv')