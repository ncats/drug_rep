...
for k in range(N):
    ...
    if sum(y_test)!=0:
        roc_auc.append(roc_auc_score(y_test, y_pro))
    if sum(y_test) ==0:
        roc_auc.append(None)

print(roc_auc)

df_SVCModelsForSingleGenes_testpred = pd.DataFrame(roc_auc)

# save results
df_SVCModelsForSingleGenes_testpred.to_csv('SVCModelsForSingleGenes_testpred.csv')