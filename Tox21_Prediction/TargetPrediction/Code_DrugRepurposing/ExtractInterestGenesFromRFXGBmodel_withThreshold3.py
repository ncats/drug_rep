...
for k in range(N):
    ...
    # print('start knn prediction on whole dataset for target gene', df_gene_single.name)
    # clf = KNeighborsClassifier(n_neighbors=33,p=2).fit(x_train, y_train)
    # y_pro = np.array(clf.predict_proba(norm_train_features))[:,1]
    # y_pro_array_knn.append(y_pro)

    print('start rf prediction on whole dataset for target gene', df_gene_single.name)
    clf = RandomForestClassifier(bootstrap = False, max_depth = 6, max_features = 'sqrt', min_samples_leaf = 2, min_samples_split = 4, 
                                 n_estimators=30).fit(x_train, y_train)
    y_pro = np.array(clf.predict_proba(norm_train_features))[:,1]
    y_pro_array_rf.append(y_pro)

    print('start xgb prediction on whole dataset for target gene', df_gene_single.name)
    clf = XGBClassifier(colsample_bytree=0.6, max_depth=3, min_child_weight=1, reg_alpha=1, subsample= 0.6).fit(x_train, y_train)
    y_pro = np.array(clf.predict_proba(norm_train_features))[:,1]
    y_pro_array_xgb.append(y_pro)

# print('knn Prediction', y_pro_array_knn)
print('rf Prediction', y_pro_array_rf)
print('xgb Prediction', y_pro_array_xgb)

df_pred = pd.DataFrame(y_pro_array_rf)
df_pred = df_pred.T
...
threshold = []
for gene in range(N):
    fpr, tpr, th = metrics.roc_curve(y_true[:,gene], y_pred[:,gene])
    optimal_idx = np.argmax(tpr - fpr)
    optimal_idx_1 = np.argmin(abs(tpr-0.7))
    optimal_threshold = th[optimal_idx]
    optimal_threshold_1 = th[optimal_idx_1]
    thre = max(optimal_threshold, optimal_threshold_1, 0.5)
    threshold.append(min(thre,0.8))

df_threshold = pd.DataFrame(threshold)
df_threshold = df_threshold.T
df_threshold.columns=col
df_threshold.index.names = ['Threshold']
print(df_threshold)

df_padding_thre = pd.DataFrame(np.repeat(df_threshold.values, len(df_difference), axis=0))
df_padding_thre.index=df_difference.index
df_padding_thre.columns=col
print(df_padding_thre.head(5))

df_subthreshold = df_difference - df_padding_thre
print(df_subthreshold.head(5))

interest_targets = df_subthreshold.gt(0).apply(lambda x: x.index[x].tolist(), axis=1)
df_interest_targets = pd.DataFrame(interest_targets,columns=['interest_gene_targets'])
print(df_interest_targets.head(5))

df_interest_gene_rf = df_interest_targets[df_interest_targets['interest_gene_targets'].astype(str) != '[]']
print(df_interest_gene_rf.head(5))

df_interest_gene_rf.to_csv('InterestGeneForCompoundsRF3.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureRF3.csv')
df_threshold.to_csv('ThresholdRF3.csv')




df_pred = pd.DataFrame(y_pro_array_xgb)
df_pred = df_pred.T
...
df_interest_gene_xgb = df_interest_targets[df_interest_targets['interest_gene_targets'].astype(str) != '[]']
print(df_interest_gene_xgb.head(5))

df_interest_gene_xgb.to_csv('InterestGeneForCompoundsXGB3.csv')
df_difference.to_csv('ProbabilityDifferencebetweenPredandTureXGB3.csv')
df_threshold.to_csv('ThresholdXGB3.csv')